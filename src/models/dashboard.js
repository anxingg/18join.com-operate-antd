import {myCity, queryWeather, query,todo,done} from '../services/dashboard'
...
export default {
  ...
  effects: {
    ...
    *todo ({ payload }, { call, put }) {
      yield put({ type: 'showLoading' ,payload:{todo:{loading:true}}})
      const {data} = yield call(todo, parse(payload))
    ...
    *done ({ payload }, { call, put }) {
      yield put({ type: 'showLoading' ,payload:{done:{loading:true}}})
      const {data} = yield call(done, (payload))
      if (data && data.status==='OK') {
        const {items,totalCount,limit,page}=data.message;
        yield put({
          type: 'querySuccess',
          payload: {
            done:{
