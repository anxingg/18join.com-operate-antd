...
function Dashboard ({dashboard, dispatch}) {
  const {todo,done,weather, sales, quote, numbers, recentSales, comments, completed, browser, cpu, user} = dashboard
  const numberCards = numbers.map((item, key) => <Col key={key} lg={6} md={12}>
    <NumberCard {...item} />
  </Col>)

  function onActive(key){
    dispatch({type: 'dashboard/'+key,payload:done.pagination})
  }
  const todoListProps = {
    ...todo,
    onPageChange(page) {
    ...
    <Row gutter={24}>
      ...
      <Col lg={18} md={24}>
        <Card bordered={false} bodyStyle={{
          ...
          <Tabs defaultActiveKey="todo" onChange={onActive}>
            <TabPane tab="代办" key="todo"><Todo {...todoListProps}/></TabPane>
            <TabPane tab="已办" key="done"><Done {...doneListProps}/></TabPane>
          </Tabs>
        </Card>
      </Col>
      <Col lg={6} md={24}>
